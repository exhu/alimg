...
proc reduceToClosest*(red: ref TColorReducer, rgba: var TRGBA, reduced: var TRGBA) {.inline.} =
    for i in 0..3'i32:
...
proc calcDiff(pd: ref TPixelDither, rgba: TRGBA, rgbaReduced: TRGBA) {.inline.}=
  for i in 0..3'i32:
      pd.rgbaDiff[i] = rgba[i] - rgbaReduced[i]


proc adjustTemp(pd: ref TPixelDither, coef: int32) {.inline.}=
  for i in 0..3'i32:
    pd.rgbaTemp[i] = clamp(pd.rgbaTemp[i] + pd.rgbaDiff[i] * coef div 16)


proc correctPixel(pd: ref TPixelDither, x, y, coef: int32) {.inline.}=
...
proc ditherImage*(pd: ref TPixelDither, img: ref TBufImg, cr: ref TColorReducer) =
    pd.img = img
